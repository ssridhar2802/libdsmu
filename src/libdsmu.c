...
static pthread_t own_server_thread; 
...
extern int clientid;
// LOCKCHANGES : BEGIN
pthread_condattr_t invalwaitca[MAX_SHARED_PAGES];
pthread_cond_t invalwaitc[MAX_SHARED_PAGES];
pthread_mutex_t invalwaitm[MAX_SHARED_PAGES];
pthread_mutex_t pteLock[MAX_SHARED_PAGES];
// LOCKCHANGES : END
...
int initlibdsmu(char *ip, int port, uintptr_t starta, size_t len, int myID) {
  ...
  int * sendP = (int *) malloc(sizeof(int));
  *sendP =myID;
  clientid = myID;
  printf("calling init owner server %d\n", myID);
  if(pthread_create(&own_server_thread, NULL, &initOwnerServer, (void *)sendP)){
